...
function AddSession() {
  ...
  const { date, location, hours, buy_in, small_blind, big_blind, cash_out } =
    formState;
  ...
  if (Auth.loggedIn()) {
    return (
      <div className="grid grid-col-1 h-screen content-center">
        ...
        <form
          className="font-sans mx-auto w-96 border-4 border-gray-900 py-4 px-6"
          type="submit"
          onSubmit={submitHandler}
        >
          <div className="grid grid-cols-1 my-5">
            <label className="text-2xl ml-3" htmlFor="date">
              Date:
            </label>
            <input
              className="py-1 text-xl bg-blue-50 border-gray-400 w-11/12 ml-3"
              type="date"
              defaultValue={date}
              name="date"
          ...
          <div className="grid grid-cols-1 my-5">
            <label className="text-2xl ml-3" htmlFor="hours">
              Hours:
            </label>
            <input
              className="py-1 text-xl bg-blue-50 border-gray-400 w-11/12 ml-3"
          ...
          <div className="grid grid-cols-1 my-5">
            <label className="text-2xl ml-3" htmlFor="buy_in">
              Buy In:
            </label>
            <div>
              <span className="text-xl">{dollar}</span>
              <input
                className="py-1 text-xl bg-blue-50 border-gray-400 w-11/12 ml-1"
                type="number"
                step="1"
                min="1"
                name="buy_in"
          ...
          <div className="grid grid-cols-1 my-5">
            <label className="text-2xl ml-3" htmlFor="small_blind">
              Small Blind:
            </label>
            <div>
              <span className="text-xl">{dollar}</span>
          ...
          <div className="grid grid-cols-1 my-5">
            <label className="text-2xl col-span-2 ml-3" htmlFor="big_blind">
              Big Blind:
            </label>
            <div>
              <span className="text-xl">{dollar}</span>
          ...
          <div className="grid grid-cols-1 my-5">
            <label className="text-2xl col-span-2 ml-3" htmlFor="cash_out">
              Cash Out:
            </label>
            <div>
              <span className="text-xl">{dollar}</span>
          ...
          <div className="mt-6">
            <button className="py-1 font-sans text-gray-800 text-2xl bg-gray-300 w-2/6 ml-3 w-11/12">
              Submit
            </button>
          </div>
        </form>
        {error && (
          <span className="grid grid-cols-1 my-5 text-xl text-gray-900 text-center">
            Add session failed
          </span>
        )}
      </div>
